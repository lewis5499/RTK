...
#define trace(fmt, ...) printf("FILE PATH: %s, LINES: %d, DEBUG OUTPUT: " fmt, __FILE__, __LINE__, ##__VA_ARGS__)

#define tracef(fmt, ...) \
    do { \
        if (cfg.enableTrace && traceLogfile) { \
            fprintf(traceLogfile, "FILE PATH: %s, LINES: %d, DEBUG OUTPUT: " fmt, __FILE__, __LINE__, ##__VA_ARGS__); \
            fflush(traceLogfile); \
        } \
    } while (0)
