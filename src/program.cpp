...
void menu()
{
	if (loadConf(PATH_CONFIG, cfg) < 0) return;
...
static void printSatLog(satres_t *sat, FILE* fp)
{
	string PRN = "";
...
static void printPosLog(pos_t *pos, FILE *fp)
{
	double blh[3];
...
static void printPos(FILE *fp, raw_t *raw, pos_t *pos)
{
...
static int posCheck(epoch_t *obs, pos_t *pos, raw_t *raw)
{
