...
void checkstat(int *stat, fd_set *set, callback_t *p) //[0]=base, [1]=rover
{
	...
	FD_ZERO(&set[0]);
	FD_SET(p->NetBas, &set[0]);
	FD_ZERO(&set[1]);
	FD_SET(p->NetRov, &set[1]);
	
	timeout.tv_sec = 0;      // Unit: microseconds
	timeout.tv_usec = 20000; // Adjust the timeout as needed (unit: milliseconds)

	stat[0] = select((int)(p->NetBas) + 1, &set[0], NULL, NULL, &timeout);
	stat[1] = select((int)(p->NetRov) + 1, &set[1], NULL, NULL, &timeout);
}
...
void rejoin(callback_t* p, char type)
{
	switch (type) 
	{
	case 0: {
...
VOID CALLBACK TimerCallback(PVOID lpParam, BOOLEAN TimerOrWaitFired)
{
	...
	if (stat[1] > 0 && FD_ISSET(p->NetRov, &readset[1]))
	{	// Rover Socket is ready for reading
		if ((p->lenRRov = recv(p->NetRov, (char*)p->buffRov, MAXRAWLEN, 0)) > 0) 
		{			
			cache(p, ROVER);

			if (stat[0] > 0 && FD_ISSET(p->NetBas, &readset[0]))
			{	// Base Socket is ready for reading
