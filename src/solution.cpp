...
static inline double svClkCorr(const double dt, const double *params) {
	return params[0] + params[1]*dt + params[2]*SQR(dt);
}

/* get sat Clock drift correction */
static inline double svClkdCorr(const double dt, const double *params) {
	return params[1] + 2 * params[2] * dt;
}

/* get earth rotation correction matrix */
static double *earthRotCorr(const double *vec3, const double time, const double omga)
{
	double rot[9];
...
static double getPIF(epoch_t *obs, const int idx, const int sys)
{
	if (sys == BDS) {
		double p1, p3;
...
static int filterspp(const double *Xk_1, const double *Pk_1, double *Xkk_1, double *Pkk_1)
{
	const double dt = 1.0;
...
static int filterspp(const double *Pkk_1, const double *Xkk_1, double *Xk, double *Pk, epoch_t *obs, const int *locs, int num)
{
...
/* deepCopy 'MWGF' type ------------------------------------------------------------------------ */
static inline void MWGFdpcy(const mwgf_t *source, mwgf_t *dst)
{
	dst->Sys = source->Sys;
	dst->Prn = source->Prn;
	dst->n = source->n;
...
static int initMWGF(epoch_t *obs, const int idx)
{
...
static void _detectOutlier(epoch_t *obs, const mwgf_t *lastEpoch)
{
...
static int initMWGF(sdobs_t *SDobs, const int idx)
{
...
static void _detectCycleSlip(sdobs_t *SDobs, mwgf_t *lastEpoch)
{
...
void detectOutlier(epoch_t *obs)
{
	...
	_detectOutlier(obs, lastEpoch);
}

void detectCycleSlip(sdobs_t *SDobs)
{
	...
	_detectCycleSlip(SDobs, lastEpoch);
}

/* make the obsdata/satPVTdata in order/accordance --------------------------------------------- */
void satInOrder(epoch_t *obs)
{
	int idx = 0, i;
...
static int searchSat(epoch_t *obs, const short prn, const int sys)
{
...
static int gpsposs(gpseph_t *ephgps, epoch_t *obs, const int prn_1, const bool isSPP, const double dt)
{
	int n, idx;
...
static int bdsposs(bdseph_t *ephbds, epoch_t *obs, const int prn_1, const bool isSPP, const double dt)
{
...
static int pgpsposs(epoch_t *obs, gpseph_t *ephgps, const int idx, const double dt)
{
	int n, ord_idx;
...
static int pbdsposs(epoch_t *obs, bdseph_t *ephbds, const int idx, const double dt)
{
	int n, ord_idx;
...
static int searchSats(epoch_t *obs, int *locs, int &num, int &numgps, int &numbds)
{
...
static int searchSats(epoch_t *obs, int *locs, int &num)
{
	num = 0;
	for (int i = 0; i < obs->SatNum; i++) //mark the valid idx
...
/* spp, Method: LS ----------------------------------------------------------------------------- */
static int lsqspp_(epoch_t *obs, const int *locs, double *Xr, double *rcvClkOft, double *Qxx, double *Dxx,
...
/* spv, Method: LS ----------------------------------------------------------------------------- */
static int lsqspv_(epoch_t *obs, const int *locs, const int num, const int nn, double *vel, double *Xr,
...
static int sppspv(epoch_t *obs, gpseph_t *ephgps, bdseph_t *ephbds, pos_t *pos)
{
	(*pos).reset();
...
static double getSDObs(const satobs_t& sati, const satobs_t& satj, int k, short flag)
{
	if (k > 1) return 0.0;
...
RECOMPUTE:
	...
	// Initialize: X
	if (!cfg.kinematic && ekfParam.IsInit) {
		matcpy(X, ekfParam.X_, 3);
	} else {
...
void checkConsistency(satobs_t *pre, satobs_t *cur, const int n_pre, const int n_cur, const double dt)
{
