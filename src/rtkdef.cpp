...
{
	int i, k, sys, sta, end, numGps, num, row=0;
	...
	for (i = 0; i < num; i++) {
		sys = (int)raw->SDObs.SDSatObs[this->Index[i][0]].sys - 1;
		sta = (sys == 0) ? 0 : numGps;
		end = (sys == 0) ? numGps : num;

		for (k=sta; k<end; k++)	{ // Pseudorange
		...
		row++;
		for (k=sta; k<end; k++)	{ // Carrier Phase
			if (this->Index[i][1]==this->Index[k][1])
...
{
	B = P = w = Q = m = X = v = NULL;
	rowB = colB = rowB = rowB = colB = colB = rowB = 0;
	mse = 0.0;
}
...
double lsq_t::lsq()
{
	...
	double *BTP  = new double [p->colB*p->rowB];
	...
	double *vTP  = new double [1*p->rowB];

	matTran(p->B, p->rowB, p->colB, BT);
	matMul(BT, p->colB, p->rowB, p->P, p->rowB, p->rowB, BTP);
	matMul(BTP, p->colB, p->rowB, p->B, p->rowB, p->colB, BTPB);
	matMul(BTP, p->colB, p->rowB, p->w, p->rowB, 1, BTPw);
	matinv(BTPB, p->colB);
	matMul(BTPB, p->colB, p->colB, BTPw, p->colB, 1, x);
	matMul(p->B, p->rowB, p->colB, x, p->colB, 1, Bx);
	matSub(Bx, p->w, v, p->rowB, 1);
	matMul(v, 1, p->rowB, p->P, p->rowB, p->rowB, vTP);
	matMul(vTP, 1, p->rowB, v, p->rowB, 1, &p->mse);

	if(p->v!=NULL) matcpy(p->v, v, p->rowB);
	p->mse /= p->rowB - p->colB;
	p->mse = SQRT(p->mse);
	for(int i=0;i<3;i++) p->m[i] = p->mse*SQRT(BTPB[i*(p->colB+1)]);
	matcpy(p->Q, BTPB, SQR(p->colB));
	matAdd(p->X, x, p->X, p->colB, 1);
	res = norm(x, 3);
